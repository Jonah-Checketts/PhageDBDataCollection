...
while True:
    ...
    if commands[0] == "help" or commands[0] == "h":
        ...
        rows = []
        with open(commands[2], "w", newline="") as f:
            col_names = ["phage_name", "genbank_access_id", "GC_percent", "num_tRNAs", "host_species"]
            writer = csv.DictWriter(f, fieldnames=col_names)
            writer.writeheader()
            for result in results:
                rows.append({"phage_name": result['phage_name'], "genbank_access_id": result['genbank_accession'], "GC_percent": result['gcpercent'], "num_tRNAs": result['num_tRNAs'], "host_species": result['isolation_host']['genus'] + ' ' + result['isolation_host']['species']})
